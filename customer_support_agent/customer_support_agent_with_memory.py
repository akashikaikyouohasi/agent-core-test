...
logger = logging.getLogger(__name__) # モジュール名をロガー名に使用。__name__は現在のモジュール名を返す
...
def invoke(payload):
    ...
    if email_match:
        #.group() は、マッチした文字列を取得するメソッド
        email = email_match.group()
        
        actor_id = f"customer_{hashlib.md5(email.encode()).hexdigest()[:8]}" # 顧客IDをMD5ハッシュで生成
        session_id = f"session_{int(time.time())}" # セッションIDをタイムスタンプで生成

        logger.info(f"顧客を識別: {email} -> {actor_id}")

        # Memory Hook付きエージェントをセットアップ
        agent, memory_client = setup_agent_with_memory(actor_id, session_id)

        # エージェントを実行（Memory Hookが自動的に処理
        ...
        # メールアドレスがない場合は、Memoryなしで処理
        logger.info("メールアドレスが見つかりません。Memoryなしで処理をします。")
        ...
        return {
            "result": str(response.message if hasattr(response, "message") else response) # hasattr()はオブジェクトが特定の属性を持っているかどうかを確認するための組み込み関数
        }
